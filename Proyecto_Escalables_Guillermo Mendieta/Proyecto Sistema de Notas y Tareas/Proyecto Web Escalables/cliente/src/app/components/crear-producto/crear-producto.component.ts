  ...
  agregarProducto(){
    ...
    if(this.id !== null){
      ...
      this._productoService.editarNota(this.id, PRODUCTO).subscribe(data => {
        this.toastr.info('Actualizado con exito', 'Pendiente Actualizado!');
      ...
      this._productoService.guardarNota(PRODUCTO).subscribe(data => {
        this.toastr.success('Registrado con exito', 'Pendiente Registrado!');
        this.router.navigate(['/']);
      }, error => {
        console.log(error);
        this.productoForm.reset();
